...
while (i < Npart)
{
   ...
   den <- integral2(post0, 0, TT, 0, 100, Tout=Tout, Yout=Yout, Dout=Dout,
                    TT=TT, admin=d, alpha=alpha, beta=beta, lambda=lambda)$Q
   num <- NULL
   for (uu in 1:nsched)
     num[uu] <- integral2(post1, 0, TT, 0, 100, Tout=Tout, Yout=Yout, Dout=Dout,TT=TT,
                          admin=d, sched=uu, alpha=alpha, beta=beta, lambda=lambda)$Q
   ptox.mean <- num/den
